...
def train(model, data_loader, optimizer, epoch, device, args):
    ...
    for i, (video_embeddings, video_mask,  _) in enumerate(
...
def main(args, config):
    ...
    for epoch in range(0, config["max_epoch"]):
        if not args.evaluate:
            ...
            train(model, [], optimizer, epoch, device, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--clip_root", type=str, default="dataset/ActivityNet/feat/vt_clipscore"
    )
    ...
    config = yaml.load(open(r"C:\Users\voanh\Downloads\New folder (35)\prompt_focus_vsum\config\vtsum_blip_tt_ca.yaml", "r"), Loader=yaml.Loader)
